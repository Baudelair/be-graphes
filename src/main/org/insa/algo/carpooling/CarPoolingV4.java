...
public class CarPoolingV4 extends CarPoolingV1 {
    public CarPoolingV4(CarPoolingData data) {
        super(data);
    }
    ...
    protected CarPoolingSolution doRun() {
    	...
    	Node B = data.getB();
    	Node D = data.getD();
    	double dist = Point.distance(graph.get(0).getPoint(), A.getPoint())+Point.distance(graph.get(0).getPoint(), B.getPoint())+Point.distance(graph.get(0).getPoint(), D.getPoint());
    	double tempDist;
    	Node C = graph.get(0);
    	for (Node n : graph){
    		tempDist = Point.distance(n.getPoint(), A.getPoint())+Point.distance(n.getPoint(), B.getPoint())+Point.distance(n.getPoint(), D.getPoint());
    		if (tempDist<dist){
    			dist = tempDist;
    			C=n;
    		}
    	...
    	while (cont == 1){
    		...
    		while (cont2 == 1 && iter.hasNext()){
    			...
    			if(cont2==1 && iter.hasNext() == false){ //si on finit de parcourir successeurs sans trouver, on tente de débloquer algo
    				...
    				int cont3 = 1;
    				int maxcompt = 100;
    				int maxrand = (int)((currSol.getPAC().size()+currSol.getPBC().size()+currSol.getPCD().size())*0.2);
